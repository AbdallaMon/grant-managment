...
export default function StudentsPage() {
    ...
    } = useDataFetcher(`sponsor/students?sponsorId=${user.id}&`, false);
              ...
              <Box display="flex" width="fit-content" gap={2} px={2} flexWrap="wrap" alignItems="center">
                  <div>
                      <SearchComponent
                            ...
                            mainKey="email"
                            localFilters={{role: "STUDENT", sponsorId: user.id}}
                    ...
                    extraComponent={({item}) => (
                          <Box sx={{display: "flex", gap: 2}}>
                              <DrawerWithContent item={item} component={StudentProfileViewer}
                                                 extraData={{route: "sponsor/students", label: "رؤية التفاصيل"}}/>
                          </Box>
                    )}
              />
