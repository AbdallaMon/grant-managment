...
export async function loginUser(email, password) {
  ...
  if (!user.password) {
    throw new Error(
      "ليس لديك كلمة مرور، من فضلك قم بإعادة تعيين كلمة المرور الخاصة بك"
    );
  }
  ...
  if (!user.emailConfirmed) {
    await generateConfirmationToken(user.id, email);
    throw new Error(
      "البريد الإلكتروني الخاص بك غير مؤكد تم ارساله ايميل لتاكيدة"
    );
  }
...
export const createUser = async (userData) => {
  const { basicInfo, contactInfo, studyInformation, bankInfo } = userData;
...
export const confirmEmail = async (token) => {
  ...
  return {
    message: "تم تاكيد حسابك بنجاح , جاري اعادة توجيهك ",
    loginUser,
    loginToken,
  };
};
