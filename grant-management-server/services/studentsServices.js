...
export const deleteDraftApplication = async (appId) => {
  const supportingFiles = await prisma.supportingFiles.findFirst({
    where: { applicationId: Number(appId) },
  });
  let deleteFiles = [];
  if (supportingFiles) {
    deleteFiles = [
      supportingFiles.personalId,
      supportingFiles.studentDoc,
      supportingFiles.medicalReport,
      supportingFiles.personalPhoto,
      supportingFiles.proofOfAddress,
    ];
  }
...
export const createDraftApplicationModel = async (appId, model, inputData) => {
  switch (model) {
    ...
    case "academicPerformance":
      if (
        inputData.gpaType === "GPA_4" &&
        (inputData.gpaValue > 4 || inputData.gpaValue < 0)
      ) {
        throw new Error(
          "المعدل التراكمي يجب ان يكون اكبر من 0 واقل من او يساوي 4 اذا كان نوعه معدل من 4 نقاط"
        );
      }
      if (
        inputData.gpaType === "PERCENTAGE" &&
        (inputData.gpaValue > 100 || inputData.gpaValue < 0)
      ) {
        throw new Error(
          "المعدل التراكمي يجب ان يكون اكبر من 0 واقل من او يساوي 100 اذا كان نوعه معدل مئوي"
        );
      }
      return await prisma.application.update({
        where: {
    ...
    case "residenceInfo":
      if (inputData.familyIncome)
        inputData.familyIncome = +inputData.familyIncome;
      if (inputData.motherIncome)
        inputData.motherIncome = +inputData.motherIncome;
      if (inputData.fatherIncome)
        inputData.fatherIncome = +inputData.fatherIncome;
    ...
    case "commitment":
      if (inputData.commitment !== true)
        throw new Error(
          "يجب عليك الموافقه في حالة كنت موافق بالفعل اعد تحميل الصفحة واكد موافقتك من فضلك"
        );
      return await prisma.application.update({
        where: { id: Number(appId), status: "DRAFT" },
    ...
    case "grantShipTerms":
      if (inputData.grantShipTerms !== true)
        throw new Error(
          "يجب عليك الموافقه في حالة كنت موافق بالفعل اعد تحميل الصفحة واكد موافقتك من فضلك"
        );
      return await prisma.application.update({
...
export const updateApplicationModel = async (appId, model, inputData) => {
  switch (model) {
    ...
    case "academicPerformance":
      let oldFileUrl = null;
      let urlsToBeDeleted = [];
      const currentAcademicPerformance =
        await prisma.academicPerformance.findUnique({
          where: { applicationId: Number(appId) },
          select: { transcript: true, gradeRecords: true },
        });
      if (
        currentAcademicPerformance &&
        currentAcademicPerformance.gradeRecords.length > 0
      ) {
        currentAcademicPerformance.gradeRecords.forEach((rec) => {
          const found = inputData.gradeRecords.some(
            (newRec) => newRec.url === rec.url && newRec.updated
          );
          if (!found) {
      ...
      if (
        ...
        typeof inputData.transcript === "string" &&
        inputData.transcript.trim() !== ""
      ) {
...
export const updatePersonalInfo = async (userId, model, updateData) => {
  ...
  } else if (model === "bankInfos") {
    updateFields.bankInfos = {
      update: updateData,
    };
  } else {
...
export const updateBankInfo = async (bankId, bankInfo) => {
  return await prisma.bankInfo.update({
    where: { id: Number(bankId) },
    data: bankInfo,
  });
};
export const createNewBankInfo = async (bankInfo) => {
  return await prisma.bankInfo.create({
    data: bankInfo,
  });
};
export const checkIfFieldsAreEmpty = async (appId) => {
  ...
  const fieldLinks = {
    supportingFiles: {
      ...
      text: "الذهاب لمليء الملفات الداعمة",
    },
    scholarshipInfo: {
      href: "scholarship-info",
      text: "الذهاب لمليء معلومات المنحة",
    },
    academicPerformance: {
      href: "academic-performance",
      text: "الذهاب لمليء الأداء الأكاديمي",
    },
    residenceInfo: {
      href: "residence-info",
      text: "الذهاب لمليء معلومات الإقامة",
    },
...
export async function updateAskedFieldsAndImprovementRequests(
  appId,
  askedFieldsData,
  improvementRequestsData
) {
...
export const getImprovementRequestsByModel = async (appId, modelName) => {
  try {
    ...
    throw new Error(
      `Failed to retrieve improvement requests: ${error.message}`
    );
  }
};
