          ...
          {!isApplication && (
            ...
            >
              <CardContent>
                ...
                <Typography>
                  الاسم:{" "}
                  {userGrants[0]?.user?.personalInfo?.basicInfo?.name ||
                    "غير متوفر"}
                </Typography>
                <Typography>
                  البريد الإلكتروني: {userGrants[0]?.user?.email || "غير متوفر"}
                </Typography>
              </CardContent>
            </Card>
          )}
                    ...
                    <Collapse in={expandedGrants[index]}>
                      ...
                      <Grid container spacing={2}>
                        <Grid item size={{ xs: 12, md: 6 }}>
                          <Typography>
                            اسم المنحة: {userGrant.grant?.name || "غير متوفر"}
                          </Typography>
                          <Typography>
                            تاريخ البدء:{" "}
                            {dayjs(userGrant.startDate).format("DD/MM/YYYY")}
                          </Typography>
                          <Typography>
                            تاريخ النهاية:{" "}
                            {dayjs(userGrant.endDate).format("DD/MM/YYYY")}
                          </Typography>
                        </Grid>
                        <Grid item size={{ xs: 12, md: 6 }}>
                          <Typography>
                            المدة بين الدفعات:{" "}
                            {PayEveryENUM[userGrant.payEvery]}
                          </Typography>
                          <Typography>
                            إجمالي المبلغ: {userGrant.totalAmounts}
                          </Typography>
                        </Grid>
                      </Grid>
                      <Typography
