...
export async function getPendingPaymentsByMonth(
  startOfMonth,
  endOfMonth,
  status = null,
  userId,
...
export async function createUserGrant(body, appId) {
  ...
  if (!application.supervisorId) {
    throw new Error(
      "هذة المنحة غير مرتبطه بمشرف يجب تعين مشرف اولا قبل اتخاذ اي اجراء"
    );
  }
...
export const getInvoiceById = async (invoiceId, userId = null, role) => {
  const invoice = await prisma.invoice.findFirst({
    where: {
      ...
      ...(role === "SUPERVISOR" &&
        userId && {
          // Apply supervisor filter only if role is SUPERVISOR
          payment: {
            userGrant: {
              supervisorId: userId,
            },
...
export async function getFixedData(type) {
  const data = await prisma.fixedData.findMany({
    where: {
      type: type,
    },
  });
...
export async function getDocuments() {
  const documents = await prisma.FixedFiles.findMany();
  return documents;
}
