...
router.put("/personal/:userId", async (req, res) => {
  ...
  try {
    ...
    if (!updatedInfo) {
      return res
        .status(404)
        .json({ message: "لم يتم العثور على المعلومات المطلوبة" });
    }
    res
      .status(200)
      .json({ message: "تم تحديث المعلومات بنجاح", data: updatedInfo });
  } catch (error) {
...
router.put("/personal/bankinfo/:bankId", async (req, res) => {
  ...
  try {
    const updatedInfo = await updateBankInfo(bankId, data);
    res
      .status(200)
      .json({ message: "تم تحديث المعلومات بنجاح", data: updatedInfo });
  } catch (error) {
    console.error("Error updating personal info:", error);
    res.status(500).json({ message: "حدث خطأ أثناء تحديث المعلومات البنكية" });
  }
});

router.post("/personal/bankinfo", async (req, res) => {
  const data = req.body; // model can be 'basicInfo', 'contactInfo', or 'studyInfo'
  const personalId = req.query.extraId;
  data.personalInfoId = Number(personalId);
  try {
    const updatedInfo = await createNewBankInfo(data);
    res
      .status(200)
      .json({ message: "تم تحديث المعلومات بنجاح", data: updatedInfo });
  } catch (error) {
...
router.get("/applications", async (req, res) => {
  ...
  try {
    const { applications, total } = await getStudentApplications(
      studentId,
      skip,
      limit
    );
    ...
    res
      .status(500)
      .json({ message: "خطأ في جلب الطلبات", error: error.message });
  }
});
router.post("/applications/draft", async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      id,
      message: "تم انشاء طلب  منحة جديدة جاري اعادة توجيهك لملئ البيانات",
    });
  } catch (error) {
...
router.post("/applications/draft/:appId", async (req, res) => {
  ...
  try {
    ...
    const createdData = await createDraftApplicationModel(
      appId,
      model,
      inputData
    );
    ...
    res
      .status(200)
      .json({ message: "تم حفظ البيانات بنجاح", data: createdData });
  } catch (error) {
...
router.put("/applications/draft/apps/siblings/:siblingId", async (req, res) => {
  ...
  try {
    ...
    const updatedData = await updateApplicationModel(
      siblingId,
      "siblings",
      inputData
    );
    ...
    res
      .status(200)
      .json({ message: "تم تحديث البيانات بنجاح", data: updatedData });
  } catch (error) {
    console.log(error, "خطأ في تحديث بيانات نموذج طلب المسودة");
    handlePrismaError(res, error);
  }
...
router.delete(
  "/applications/draft/apps/siblings/:siblingId",
  async (req, res) => {
    const { siblingId } = req.params;
    try {
      ...
      res
        .status(200)
        .json({ message: "تمت عملية الحذف بنجاح", data: updatedData });
    } catch (error) {
      console.log(error, "خطأ في تحديث بيانات نموذج طلب المسودة");
      handlePrismaError(res, error);
    }
...
router.get("/applications/:appId/submit", async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      message: "تم مراجعة بياناتك والتاكد ان كل البيانات مكتملة",
      data: [],
    });
  } catch (error) {
...
router.get("/applications/:appId/submit/uncomplete", async (req, res) => {
  ...
  try {
    ...
    return res.status(400).json({
      message:
        "لديك بعض البيانات التي بحاجه الي تعديل او المشرف يريد المزيد من الاضافات",
      data,
    });
  } catch (error) {
    res.status(500).json({ message: "حدث خطأ", error: error.message });
...
router.get(
  "/applications/:appId/improvement-requests-model/:model",
  async (req, res) => {
    const { appId, model } = req.params;
    try {
      const improvementRequests = await getImprovementRequestsByModel(
        appId,
        model
      );
      ...
      if (!improvementRequests.length) {
        return res
          .status(200)
          .json({ message: "لا توجد طلبات تحسين لهذا النموذج.", data: [] });
      }
      res.status(200).json({
        message: "تم جلب طلبات التحسين بنجاح.",
...
router.post("/applications/:appId/submit/uncomplete", async (req, res) => {
  ...
  const body = req.body;
  try {
    const submittedApplication = await updateAskedFieldsAndImprovementRequests(
      appId,
      body.askedFields
    );
    ...
    await createNotification(
      supervisorId,
      `تم تحديث طلب منحة من قبل الطالب (معرف الطلب:#${appId})`,
      `/dashboard/apps/view/${appId}/${studentId}`,
  ...
  try {
    ...
    if (!application) {
      if (status === "DRAFT") {
        return res
          .status(404)
          .json({ error: "هذا الطلب غير موجود او تم ارساله للمشرف" });
      } else if (status === "UN_COMPLETE") {
        return res
          .status(404)
          .json({ error: "هذا الطلب تم تحديثة وسيتم مراجعته م قبل الادمن" });
      } else if (status === "APPROVED") {
        return res
          .status(404)
          .json({ error: "غير مصرح لك برؤية هذا الطلب في الوقت الحالي" });
      }
    }
    res.status(200).json({ data: application });
  } catch (error) {
...
router.post("/applications/:appId/updates", async (req, res) => {
  ...
  try {
    ...
    await createNotification(
      supervisorId,
      `تم اضافة تحديث جديد علي طلب منحة من قبل الطالب (معرف الطلب:#${appId})`,
      `/dashboard/apps/view/${appId}/${studentId}`,
      "APPLICATION_UPDATE"
    );
...
router.get("/dashboard/grants-stats", async (req, res) => {
  try {
    ...
    const totalAmountLeft =
      totalPaymentsResult._sum.amount - totalPaymentsResult._sum.amountPaid;
...
router.get("/tickets", async (req, res) => {
  try {
    ...
    const { limit, skip } = getPagination(req);
    const [tickets, total] = await getTicketsByUser(
      userId,
      +skip,
      +limit,
      req.query
    ...
    res
      .status(500)
      .json({ message: "خطأ في جلب التذاكر.", error: error.message });
  }
});
...
router.post("/tickets", async (req, res) => {
  try {
    ...
    await createNotification(
      null,
      `تم انشاء تذكرة جديدة بعنوان ${title}`,
    ...
    res
      .status(500)
      .json({ message: "خطأ في إنشاء التذكرة.", error: error.message });
  }
});
...
router.get("/tickets/:ticketId/messages", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "خطأ في جلب الرسائل.", error: error.message });
  }
});
...
router.post("/tickets/:ticketId/messages", async (req, res) => {
  try {
    ...
    await createNotification(
      null,
      `رد جديد علي تذكرة #${ticketId} : ${content}`,
