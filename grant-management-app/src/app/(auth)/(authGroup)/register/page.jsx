...
const stepsIds = ["basicInfo", "studyInformation", "contactInfo", "bankInfo"];
const stepInputs = [
  [
    {
      ...
      pattern: {
        ...
        pattern: {
          value: /^[^\s]+(?:\s)?$/,
          message: "الاسم يجب أن يكون كلمة واحدة فقط",
        },
      },
    },
    {
      ...
      pattern: {
        ...
        pattern: {
          value: /^[^\s]+(?:\s)?$/,
          message: "اسم الأب يجب أن يكون كلمة واحدة فقط",
        },
      },
    },
    {
      ...
      pattern: {
        ...
        pattern: {
          value: /^[^\s]+(?:\s)?$/,
          message: "اسم العائلة يجب أن يكون كلمة واحدة فقط",
        },
      },
    },
    {
  ...
  [
    {
      ...
      pattern: {
        required: { value: true, message: "نوع البرنامج الدراسي مطلوب" },
      },
    },
    {
      data: {
    ...
    },
    {
      ...
      pattern: {
        required: { value: true, message: "رقم الطالب الجامعي مطلوب" },
      },
    },
  ],
  [
  ...
  const onSubmit = async (data) => {
    if (activeStep < steps.length - 1) {
      ...
      setFormData((prevData) => ({
        ...prevData,
        [stepsIds[activeStep]]: sectionData,
      }));
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale={locales}>
      ...
      >
        <Container
          maxWidth="md"
          sx={{ p: 0, height: "100%", overflowY: "auto" }}
        >
          <Button
            variant="contained"
            href="/login"
              ...
              <>
                <form onSubmit={handleSubmit(onSubmit)}>
                  <Grid container spacing={2}>
                    {activeStep < stepInputs.length &&
                      stepInputs[activeStep].map((input) =>
                        input.data.type === "SelectField" ? (
                          ...
                          <Grid size={12} key={input.data.id}>
                            <ResidenceAndNationalitySelectors
                              errors={errors}
                              control={control}
                              setValue={setValue}
                              register={register}
        ...
        <Box key={step} sx={{ mb: 4 }}>
          ...
          <Grid container spacing={2}>
            {inputs[index].map((item) => (
              <>
                {item.data.type !== "password" &&
                  item.data.type !== "confirmPassword" &&
                  item.data.type !== "disability" &&
                  item.data.type !== "country" && (
                    <>
                      <Grid size={{ xs: 12, md: 6 }} key={item.data.id}>
                        <Typography variant="body1">
                          <strong>{item.data.label}:</strong>{" "}
                          {data[step][item.data.id]}
                        </Typography>
                      </Grid>
                    </>
                  )}
            ...
            {index === 0 && (
              <>
                <Grid size={6}>
                  <Typography variant="body1">
                    <strong>بلد الاقامة:</strong>{" "}
                    {data.basicInfo.residenceCountry}
                  </Typography>
                  <Typography variant="body1">
                    <strong>المدينة:</strong> {data.basicInfo.nationality}
                  </Typography>
                </Grid>
                <Grid size={6}>
                  <Typography variant="body1">
                    <strong> لدية اعاقة؟ :</strong>{" "}
                    {data.basicInfo.hasDisability === "yes" ? "نعم" : "لا"}
                  </Typography>
                  <Typography variant="body1">
                    <strong>تفاصيل الاعاقة:</strong> {data.basicInfo.disability}
                  </Typography>
                </Grid>
              </>
            )}
