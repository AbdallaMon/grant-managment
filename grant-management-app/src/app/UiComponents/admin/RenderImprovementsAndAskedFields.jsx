...
export const RenderImprovementsAndAskedFields = ({
  item,
  route,
  view,
  isStudent,
  ...
  const fetchData = async (key, setData) => {
    ...
    const request = await fetch(
      `${process.env.NEXT_PUBLIC_URL}/${apiRoute}/${item.id}/${key}`,
      {
        credentials: "include",
      }
    );
  ...
  const renderStatusIcon = (status) => {
    switch (status) {
      case "APPROVED":
        return (
          <CheckCircleIcon color="green" size={20} style={{ marginLeft: 4 }} />
        );
      case "REJECTED":
    ...
    <Box dir="rtl">
      ...
      {!isStudent && (
        <Alert severity="info" sx={{ my: 2 }}>
          ملاحظة: التعديلات أدناه تم إرسال طلبها من قبلك أو من قبل المشرف إلى
          الطالب لتعديلها أو لإضافة حقل معين.
        </Alert>
      )}
      <Accordion
...
function CreateNewUpdate({ appId, setData }) {
  ...
  async function submit(data) {
    ...
    const request = await handleRequestSubmit(
      ...
      setLoading,
      `student/applications/${appId}/updates`,
      false,
      "جاري اضافة التحديث"
    );
