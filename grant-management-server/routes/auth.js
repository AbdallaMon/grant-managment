...
router.post("/register", async (req, res) => {
  try {
    ...
    res.status(200).json({
      status: 200,
      message:
        "تم تسجيل الحساب بنجاح، يرجى التحقق من بريدك الإلكتروني لتأكيد حسابك",
      user,
    });
  } catch (error) {
...
router.get("/status", (req, res) => {
  ...
  if (!token) {
    return res
      .status(401)
      .json({ auth: false, message: "أنت لست مسجلاً للدخول" });
  }
  try {
...
router.post("/confirm/:token", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ status: 200, message: confirm.message, user: confirm.loginUser });
  } catch (error) {
    handlePrismaError(res, error);
  }
});
