...
export default function ReviewSubmissionPage({ params: { id } }) {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      validateForm(
        response.data.askedFields,
        response.data.improvementRequests
      );
    }
  ...
  const handleAskedFieldChange = (index, value) => {
    ...
    validateForm(updatedFields, improvementRequests);
  };

  const validateForm = (askedFields, improvementRequests) => {
    const allFieldsFilled = askedFields.every((field) => field.value);
    const allRequestsChecked = improvementRequests.every(
      (request) => request.checked
    );
    console.log(askedFields);
    console.log(improvementRequests);
  ...
  const handleSaveAndSubmit = async () => {
    if (!isFormValid) {
      setSnackbarError(
        "يرجى تعبئة جميع الحقول المطلوبة وتأكيد جميع طلبات التحسين."
      );
      return;
    }

    const fileFields = askedFields.filter((field) => field.type === "FILE");
    let submittedAskedFields = askedFields.filter(
      (field) => field.type !== "FILE"
    );
    if (fileFields.length > 0) {
      submittedAskedFields = await handleBeforeUpdate(
        submittedAskedFields,
        fileFields
      );
    }
    const request = await handleRequestSubmit(
                    ...
                    <Box key={index} sx={{ mb: 3 }}>
                      ...
                      {field.type === "TEXT" && (
                        <TextField
                          ...
                          value={field.value || ""}
                          onChange={(e) =>
                            handleAskedFieldChange(index, e.target.value)
                          }
                          sx={{ my: 2 }}
                        />
                      )}
                      {field.type === "FILE" && (
                        <TextField
                          ...
                          fullWidth
                          onChange={(e) =>
                            handleAskedFieldChange(index, e.target.files)
                          }
                          sx={{ my: 2 }}
                        />
                      )}
