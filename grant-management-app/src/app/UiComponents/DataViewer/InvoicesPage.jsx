...
const InvoicesPage = ({isSupervisor = false}) => {
    ...
    const fetchInvoices = async () => {
        ...
        try {
            const response = await getData({url: `shared/invoices?${query}&`, setLoading});
                                    ...
                                    {invoices?.length > 0 ? (
                                          invoices.map((invoice) => (
                                                <TableRow key={invoice.id}>
                                                    ...
                                                    <TableCell>{dayjs(invoice.dueDate).format('DD/MM/YYYY')}</TableCell>
                                                    <TableCell>{dayjs(invoice.paidAt).format('DD/MM/YYYY')}</TableCell>

                                                    <TableCell>{invoice.amount}</TableCell>
                                                    <TableCell>
                                                        {invoice.payment.userGrant.user.personalInfo.basicInfo.name || 'غير متوفر'}
                                                    </TableCell>
                                                    <TableCell>{invoice.payment.userGrant.grant.name || 'غير متوفر'}</TableCell>
                                                    <TableCell>
                                          ...
                                          <TableRow>
                                              <TableCell colSpan={7} align="center">لا توجد فواتير.</TableCell>
                                          </TableRow>
                                    )}
                                </TableBody>
                            </Table>
